...
async def get_local_models(
    ...
    return {"data": [m.to_dict() for m in models], "meta": {"cache_status": "hit" if catalog._is_cache_valid() else "miss"}}


@router.post("/api/models/install")
async def install_model(model_name: str, current_user: str = Depends(get_current_user), catalog: CatalogService = Depends(get_catalog_service)):
    """Trigger model installation. Rate limited: 5/hour."""
    # Validate model name
    safe_name = catalog._validate_model_name(model_name)

    return {"status": "queued", "model": safe_name, "websocket_channel": f"install-progress:{safe_name}"}
